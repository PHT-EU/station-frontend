...
export default {
    ...
    methods: {
        ...
        async getRunInformation() {
            ...
            for (const run_id_index in this.run_ids) {
                const run_id = this.run_ids[run_id_index];
                const url = `${process.env.VUE_APP_STATION_API}/localTrains/getAirflowRun/${run_id}`;
